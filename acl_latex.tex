...
\begin{table*}[]
\centering
\caption{English test set results of each model in Track B}
...
%The CRC model is trained via SFT on a combination of ST and CRC datasets. 
...
For the CRC model, we analyze track A anger label's bad case. We found that most misclassifications—accounting for 70\% of the errors—were due to the model incorrectly predicting a neutral emotional state. Table\ref{tab:CRC_badcases} presents five randomly sampled bad cases, illustrating that these instances are on the borderline of the anger definition. Comparing them with other samples can easily influence their predictions, causing uncertainty and error. This suggests that the dataset may not be well-suited for sample-based comparison approach.

% 在DPO模型上，我们着重分析了track B Sadness标签的bad case。 我们发现所有错误同ground truth仅有程度为1的差异。这意味着模型对于标签intensity的感知的敏感性依然存在提升的空间。
For the DPO model, we analyzed sadness misclassifications in Track B. We observed that over 90\% of the errors differed from the ground truth by only one intensity level. This indicates that the model still has room for improvement in its recognition of label intensity.
